...
def item_cf(user_id, item_id, similar_items, user_to_rating):
    ...
    for similar_item,similarity in similar_items[item_id]:
        # 相似电影的打分
        similar_score = user_to_rating[user_id][similar_item - 1]
        score += similar_score * similarity
        if similar_score != 0:
            count += 1
    score /= count + 1e-5
...
if __name__ == "__main__":
    user_item_score_data_path = r"./u.data"
    item_name_data_path = r"./u.item"
    ...
    while True:
        user_id = int(input("输入用户id："))
        recommands1,recommands2 = movie_recommand(user_id, similar_user, similar_items, user_to_rating, item_to_name)
        print("基于cf-user")
        for recommand, score in recommands1:
