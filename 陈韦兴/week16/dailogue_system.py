...
class DailogueSystem:
    ...
    def load_scenario(self, scenario_file):
        ...
        listen_again_node_id = "relisten_node"
        listen_again_node = {"id":listen_again_node_id,"intent":["你说什么","没听清楚","再说一遍"]}
        self.scenario.append(listen_again_node)
        for node in self.scenario:
            ...
            if "childnode" in node:
                ...
                node["childnode"].append(scenario_name + listen_again_node_id)
    ...
    def dst(self, memory):
        ...
        if "relisten_node" not in memory["hit_node"]:
            slot_list = self.nodes_info[memory["hit_node"]].get("slot", [])
            for slot in slot_list:
    ...
    def nlg(self, memory):
        ...
        if memory["policy"] == "reply":
            ...
            # 如果匹配到重听就重说刚刚的内容
            memory["response"] = response if response is not None else memory["response"]
        else:
